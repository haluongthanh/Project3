...
exports.AuthGoogle = asyncHandler(async (req, res, next) => {
    const { email, name, authGoogleId, } = req.body;
    const cookies = req.cookies;
    let user = await User.findOne({ email: email, authGoogleId: authGoogleId });
    if (user) {
    ...
    user = await User.create({ email, name, authGoogleId, authType: "google" })

    if (user) {
        await user.save();
        let user = await User.findOne({ email: email, authGoogleId: authGoogleId, authType: "google" });
        let newRefreshTokenArray = user.refreshToken ? user.refreshToken : [];
        const accessToken = getAccessToken(user);
        const newRefreshToken = getRefreshToken(user);
        if (cookies?.jwt) {
