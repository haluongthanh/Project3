...
const MyOrders = () => {
    ...
    useEffect(() => {
        dispatch(getMyOrders({ status, toast }));
    }, [dispatch, status]);
    ...
    return (
        <>
            {loading ? <BoxShadowLoader /> :
                <div class="right-main-box tab-content customers-orders-history current">
                    <div className="box-heading"><h2>Quản lý đơn hàng</h2></div>
                    <div className="box-info-account">
                        <div className="history-status">
                            <div className="status-list">
                                <a className={`status-item ${status === '' ? 'active' : ''}`} data-stt="all" onClick={() => handleStatusClick('all')}>
                                    <span className="text">TẤT CẢ</span><span className="count"></span>
                                </a>

                                <a className={`status-item ${status === 'Processing' ? 'active' : ''}`} data-stt="Processing" onClick={() => handleStatusClick('Processing')}>
                                    <span className="text">ĐANG XỬ LÝ</span><span className="count d-none"></span>
                                </a>
                                <a className={`status-item ${status === 'Shipped' ? 'active' : ''}`} data-stt="Shipped" onClick={() => handleStatusClick('Shipped')}>
                                    <span className="text">ĐANG VẬN CHUYỂN</span><span className="count d-none"></span>
                                </a>
                                <a className={`status-item ${status === 'Delivered' ? 'active' : ''}`} data-stt="Delivered" onClick={() => handleStatusClick('Delivered')}>
                                    <span className="text">HOÀN THÀNH</span><span className="count d-none"></span>
                                </a>
                                <a className={`status-item ${status === 'Cancel' ? 'active' : ''}`} data-stt="Cancel" onClick={() => handleStatusClick('Cancel')}>
                                    <span className="text">HUỶ</span><span className="count d-none"></span>
                                </a>
                            </div>
                        </div>
                        <div className="history-table" data-table="all">
                            {orders.length === 0 ? (
                                <div className="data-account__empty">
                                    ...
                                    <p className="alert-empty">Quý khách chưa có đơn hàng nào.</p>
                                    <div>
                                        <p class="link-continue"><a class="button" href="/"><i class="fa fa-reply"></i>Tiếp tục mua hàng</a></p>
                                    </div>
                                </div>
                            ) : (
                                <>
                                    {orders && orders.map(order => (
                                        <div class="history-table__item ">
                                            <div class="history-table__item-head">
                                                <div class="stt-order">
                                                    <span>{order.orderStatus}</span>
                                                </div>
                                                <div class="code-order">#{order.orderCode}</div>
                                            </div>
                                            {
                                                order && order.orderItems.map(item => (
                                                    <div class="history-table__item-body" data-count="1">
                                                        <div class="history-table__item-line ">
                                                            <div class="left">
                                                                <div class="img-line">
                                                                    <img src={BASEURL + item.product.images[0].url} alt="" />
                                                                    <div class="qty-line">x{item.quantity}</div>
                                                                </div><div class="info-line">
                                                                    <div class="name-line">{item.product.title}</div>
                                                                </div></div><div class="right"><div class="text-right">
                                                                    <div class="price-line">{formatCurrency(item.price)} </div>
                                                                    {/* <div class="price-original-line">395.000₫</div> */}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="history-table__item-foot">
                                                ...
                                                <div class="view-order text-right">
                                                    <a href={`order/${order._id}`} data-id="1615250031" title="Xem chi tiết">Xem chi tiết</a>
                                                </div>
                                            </div>
                                        </div>
                                    ))
